...
public class ConnectionHandler {
	 ...
	 */
	private ConnectionHandler(SocketChannel sChannel, ReactorData data, SelectionKey key, user.User user) {
		_sChannel = sChannel;
		_data = data;
		_protocol = _data.getProtocolMaker().create(user);
	...
	public static ConnectionHandler create(SocketChannel sChannel, ReactorData data, SelectionKey key, user.User user) {
		ConnectionHandler h = new ConnectionHandler(sChannel, data, key, user);
		h.initialize();
		return h;
	}
	...
	public void sayToMe(String say) {
		...
		try {
			ByteBuffer bytes = _tokenizer.getBytesForMessage(say);
	        addOutData(bytes);
